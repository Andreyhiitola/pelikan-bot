...
def generate_text_report(analytics: Dict) -> str:
    ...
    if analytics['category_averages']:
        ...
        if cat.get('avg_cleanliness', 10) < threshold:
            problems.append(f"🧹 Чистота ({cat['avg_cleanliness']:.1f})")
        if cat.get('avg_comfort', 10) < threshold:
            problems.append(f"🛏️ Комфорт ({cat['avg_comfort']:.1f})")
        if cat.get('avg_location', 10) < threshold:
            problems.append(f"📍 Расположение ({cat['avg_location']:.1f})")
        if cat.get('avg_facilities', 10) < threshold:
            problems.append(f"🏊 Удобства ({cat['avg_facilities']:.1f})")
        if cat.get('avg_staff', 10) < threshold:
            problems.append(f"👥 Персонал ({cat['avg_staff']:.1f})")
        if cat.get('avg_value', 10) < threshold:
            problems.append(f"💰 Цена/качество ({cat['avg_value']:.1f})")
